...
<script type="text/javascript">
	...
	function openNode2()
	{
		document.forms['languesForm'].submit();
	}
	function openNode3()
	{
		document.forms['languesForm'].id.value=0;
		document.forms['languesForm'].submit();
	}
	...
	function arrangeNodeStopMove()
	{
		...
		if(arrNodesDestination==subs[subs.length-1] && trash==true){
			...
			savedChange = false;
			arrNodesDestination = false;
			document.location.replace("DeleteLangue.servlet?id="+arrTarget.id);
			return;
		}
	...
	function saveArrangableNodes()
	{
		...
		document.forms['langues'].hiddenNodeIds.value = string;
...
<h1 id="titrePage">Langues</h1>
<h2>Choisissez l'ordre des langues présentées dans le formulaire</h2>
<div style="float: right; margin-left: 40%; width: 40%; height: 400px; border: 6px solid gray;">
<form action="index_agenda.jsp" method="get" name="languesForm">
<input type="hidden" name="page" value="langues"/>
<fieldset>
<select style="width: 100%;" onchange="openNode2();" name="id" >
		<option value="-1" <%= (id==-1)?"selected='selected'":"" %>">----</option>
		<option value="0" <%= (id==0)?"selected='selected'":"" %>">Ajouter une langue</option>
		<%
		...
		{
		%>
			<option value="<%= langue.getId() %>" <%= (id==langue.getId())?"selected='selected'":"" %>"><%= langue.getNom() %></option>
		<%} %></select>
</fieldset>
</form>
<div style="float: right; width: 100%; height: 300px; border: 3px solid gray;">
<form name='langueForm' id='langueForm' action='SaveLangue.servlet' method='post'>
<div id="languesForm">
	<%
	out.flush();
	if(id!=-1)
	pageContext.getServletContext().getRequestDispatcher("/LoadLangue.servlet?id="+id).include(request, response);
	out.flush();
...
<p>
	<a href="#" onclick="javascript:openNode3()">Ajouter une langue</a>
</p>
...
<form method="post" action="SaveOrdreLangues.servlet" name='langues' method="post">
