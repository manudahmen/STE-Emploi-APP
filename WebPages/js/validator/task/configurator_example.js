...
function checkEntreprise()
{
	if(configureValidation1entreprise()) { 
			document.forms['EntrepriseForm'].submit(); 
		}
}
function configureValidation1entreprise(){
    f = null;
	f = document.forms['EntrepriseForm']; //the form must be set here
	...
	f.secteur.optional = true;
	//deal with radio and check buttons must be set according to page specifics
	var preCheck = true;
	return validateForm(f,preCheck,'configureValidation1entreprise');
}
